...
| # | Item | Status | Note |
|---|------|--------|------|
| 1 | **Calendly links** | 🟡 | **1:1 + Orgs done** (`coach-everydayfuture/1-1-discovery-session` + `orgs-teams-discovery`, 2026-06-11). Still placeholder: `group` (`placeholder-15`); self-led has none by design. **Needs Taylor's 1 remaining URL** (group). Edit `options.ts` only. |
...
1. **Calendly** (#1) — 1:1 + Orgs done; still need the Group URL.
