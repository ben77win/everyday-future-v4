...
### Decisions log
- **2026-07-03 (GSC "not indexed" alert — verified benign, no action)** — Search Console emailed "New reasons preventing your pages from being indexed" listing **"Page with redirect"** (`http://everydayfuture.work/` + `https://www.everydayfuture.work/`) and **"Alternate page with proper canonical tag"** (`https://www.everydayfuture.work/home`). **Full live audit confirmed all three are intentional duplicate variants working as designed:** http→https 301, www→apex 301 (deliberate cutover config), and the old Squarespace `/home` 301-chaining (2 hops) to the homepage whose canonical Google correctly consolidated. All 8 sitemap pages verified 200 + self-canonical + no noindex; sitemap/robots clean. **Zero real pages blocked — these GSC statuses are expected permanently; do not "fix" them** (removing the redirects would create true duplicate-content problems). Note: a validation run started 7/3 on "Page with redirect" — it will report "failed" since the URLs still (correctly) redirect; cosmetic, ignore, and don't click Validate on these categories. Remaining 5 sitemap pages simply await first crawl (~2.5-week-old domain); optional GSC request-indexing nudge deferred.
- **2026-06-22 (Writing link hidden — kept indexed)** — Per Ben, the **Writing** link was hidden from all site chrome (desktop nav, mobile menu, footer — `Nav.astro` ×2 + `Footer.astro`) until more essays publish (~2 weeks). **Decision: links-only hide, keep indexed** — the `/writing` index + 2 published essays stay live, **in `sitemap-index.xml`, in `/rss.xml`, and indexed in Google (Search Console)**; robots/canonicals/schema all unchanged. Rationale: the essays were freshly indexed at launch; `noindex` + sitemap/RSS removal would risk Google dropping them and force slow re-indexing on relaunch — worse than leaving thin-but-valid pages indexed and merely unlinked from the chrome for two weeks. No SEO artifacts changed; only the internal links were removed. Committed `d875945` → `staging`. Full detail: CONTEXT.md session 10.
