...
_**Next / open:** Squarespace site safe to decommission (keep the domain registration + Google Workspace MX at Squarespace). Optional: re-run Google Rich Results + LinkedIn/X inspectors on the production domain (done on staging). Deferred launch decisions still open: bio images (keep/swap), Waymaker S5 images (needs spec). Post-launch (now that DNS is live): GA, Google Business Profile, submit sitemap to Search Console (`LAUNCH.md` §3)._
