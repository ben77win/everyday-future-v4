...
_Last updated: 2026-09-09 (**session 14 — Client Portal link reintroduced, LIVE on production**). The nav Client Portal link (desktop boxed `CLIENT PORTAL ↗` + mobile menu `Client Portal 03`), hidden 2026-08-12 because the portal root landed on the admin screen, is **un-hidden exactly as commented** (`Nav.astro`, commit `1a0393c`) and promoted to `main` with Ben's approval — verified live on everydayfuture.work. Safe now because the portal (separate repo `ben77win/edf-portal`) shipped its magic-link login and, this session, its root was changed to SERVE the Sign-In screen at `/` via a 200 rewrite (was a 302 to `/login.html`; Ben: "just root — the magic login screen") — verified: portal root returns 200 "Every Day Future — Sign In". Both repos: staging/main aligned, single-file diffs verified before promote. Legal-page v5 typography alignment (session-14 assessment: white→ivory ground + 5 metric snaps) remains RECOMMENDED, not yet approved/built._

---

_Last updated: 2026-08-13 (**session 13 — v5 MERGED TO PRODUCTION**). **everydayfuture.work now serves the v5 redesign** (merge commit `748c1c2`, Ben approved; the one divergent main commit was the share-card hotfix `d609b1e`, merged cleanly). Verified live post-deploy: all 8 routes 200; v5 hero markup; meta description with "Taylor Winters"; positioning h3; FAQPage schema; GA4; the stacked-tagline og-image (hash `0d608259`). `v5`/`staging`/`main` are now identical. **Rollback:** `git revert -m 1 748c1c2` on main, or Netlify → publish any pre-merge deploy, or hard-reset to tag `pre-v5`. **Post-launch list lives in [`V5-TODO.md`](V5-TODO.md):** Taylor's Process copy (flip `SHOW_BODY_COPY`), Writing links still hidden (SEO orphaning — decide soon), GSC re-check at ~1 and ~4 weeks (expect a brief impressions wobble; baseline 369 impressions / pos 18.9), GBP reviews (A4), definitional page (B3), credentials schema (B6), sketch-mark vectorization (5 rasters remain), portal reskin + redirect, and Ben's hand-delete cleanup pass ([`CLEANUP.md`](CLEANUP.md) — includes 3 publicly reachable `_comp-*.html` files)._

---

_Last updated: 2026-08-12 (**session 12 — Ben's punch-list pass; ALL items closed**). Work continues on branch **`v5`**, mirrored to **`staging`** on Ben's word; `main` untouched (rollback tag `pre-v5`). **Two tracking docs at the repo root: [`V5-PUNCHLIST.md`](V5-PUNCHLIST.md) (Ben's review list — every item done/ratified) and [`V5-TODO.md`](V5-TODO.md) (what stands between here and production: blocked-on-people, SVG marks, SEO confirmations, real-device verification, deferred design).** Shipped this session: **CTAs** — both pills to best practice (52px desktop / 44px mobile, matching the menu pill + Begin submit); **load screen** kept but first-visit-only via localStorage, plus a fix for the browser scroll-restoration bug that landed users mid-page; **S1** blue-inversion card hover (the comp's yellow→yellow never read at a glance) and two new images (IMG_0182 paperclip, sunset figure); **Process** per-banner mobile focal crops, and the lorem body copy hidden behind a single `SHOW_BODY_COPY` switch at the top of `Process.astro` (copy retained beside it — flip to `true` when Taylor's lands); **Testimonials** continuous rAF drift with cloned cards for a seamless wrap (never pauses except mid-drag), stars now only on result cards; **Someday** clock repositioned + halved, figure slot now the IMG_1357 water video, and a fix for it rendering twice on mobile (the mobile `display:block` was un-hiding the poster still); **Waymaker** kept (Ben's call — it carries the SEO positioning phrase); **Bio** portrait → DSCF0065 square top-anchored crop (feet out), slot unchanged; **FAQ** row hover per Ben's handoff — surface fades in over 200ms with no layout shift, switched from blue to brand yellow with ink text (17.2:1 contrast); **Footer** fading tagline stack removed (gradient kept); **Nav** Client Portal hidden pending the portal's root-redirect fix; **anchors** verified. **Marks:** 4 of 9 blue sketches are now true vector from `public/marks/` (underline `Vector-12`, oval `Vector-5`, Taylor lasso `Vector-8`, FAQ spiral `Vector-4`); the rest stay raster — **a 4x Figma re-export was tried and REVERTED after measurement proved it upscaled a low-res photographed source and came out softer (edge width 8.99px vs 3.97px)**; the remaining rasters were instead reduced in size to raise effective density. **Videos:** all four normalised to H.264 High / level 3.0 / `yuv420p` / tv-range / faststart for consistent iOS rendering. **SEO re-verified against the v5 build:** meta description with "Taylor Winters", the positioning `<h3>`, "conscious executive coaching" ×9, FAQPage/Person/ProfessionalService schema, canonical, GA4, and all 8 sitemap URLs intact._
