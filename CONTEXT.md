...
- [ ] Load screen plays on production homepage **desktop only** (phones ≤640px skip it entirely — Ben 2026-08-10); deep links (`#faq`, `?option=`) skip it; after the intro the page always releases at the HERO (reload from mid-page must not land mid-page — `history.scrollRestoration` is held at `manual`).
