...
- **S2:** the section becomes `display: grid` (`grid-template-columns: minmax(0, 1fr)`) and reorders via `order` to **header (0) → tier tiles (1) → option cards (2)**; section bottom padding 88px. The `.s2__header` keeps its full-bleed cream band (negative margin/padding — not a flex change). **Tier tiles** sit ABOVE the cards, `flex: 1`, **square `aspect-ratio: 1/1`**, **`gap: 4px`**, `max-width: none` (no scroll/snap — replaced the old 72vw snap carousel); tier number + label shrink to 11px/0.08em, label nudged to `bottom: 4px`. **Option cards** (`.s2__card`) are 1-up, `aspect-ratio: auto`, fixed `height: 140px`, `padding: 20px 28px`, `.s2__card-label` 25px; `:active` → `--blue`.
