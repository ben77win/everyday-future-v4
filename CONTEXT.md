...
_Last updated: 2026-06-17 (**session 9 — domain cutover, IN PROGRESS**). **Ben approved the `staging → main` merge ("merge as-is now") — current staging build ships to production; bio/Waymaker image decisions deferred (non-blocking).** Cutover is email-safe by design: DNS stays at Squarespace, only the web records move. Confirmed via `dig`: **email = Google Workspace** (MX `aspmx.l.google.com` + alts — MUST NOT be touched); **DNS hosted at Squarespace** (NS `squarespacedns.com` + `nsone.net`); registrar Tucows/OpenSRS. Old Squarespace site = 3 pages (`/home`, `/rei-internal`, `/client-referral`). Session 9 work:_
_1. **301 redirects built** — `public/_redirects`: `/home`, `/rei-internal`, `/client-referral` → `/` (301, per Ben "all → homepage"). Build-verified in `dist/`; none of the 3 paths collide with real pages._
_2. **`staging → main` merge** — fast-forward (main was strictly behind; no divergence). Production Netlify site now reflects the current build; **public domain still on Squarespace until DNS flips.**_
_3. **DNS cutover (Ben's manual steps, Claude verifies):** add `everydayfuture.work` (primary) + `www` in Netlify → SSL pre-stages; lower TTL ~300s; then in the **Squarespace DNS panel** change apex `A` (4 Squarespace IPs → Netlify `75.2.60.5`, or whatever Netlify's UI shows) + `www` CNAME (`ext-sq.squarespace.com` → `everyday-future-v4.netlify.app`); **leave all MX/TXT untouched.** Netlify auto-301s `www`→apex. Keep Squarespace live as instant rollback (revert the 2 records). Full playbook: `LAUNCH.md` §2 + `SEO.md` Domain Cutover Checklist._
_**Next:** Ben executes the Netlify + Squarespace DNS steps; Claude dig-verifies (web → Netlify, MX identical, HTTPS valid, 301s + OG/Rich Results on the live domain)._
