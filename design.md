...
- Centered wordmark + right-anchored "Writing" + "Client Portal" links (`.nav__right`, hidden ≤640px). **⏸ "Writing" link temporarily hidden 2026-06-22** (Astro `{/* */}` comment in `Nav.astro`) until more essays publish (~2 weeks) — the `/writing` section stays live + indexed; only "Client Portal" renders in `.nav__right` for now. See CONTEXT.md session 10.
...
- Step 2 — **anchored split (2026-06-11 redesign, handoff 6; supersedes the session-6 layout):**
  ...
  - **Submit:** **full-panel-width, min-height 52px**, ink → blue hover, Mono 14.4/0.110em, 4px radius. Booking label **"Continue to scheduling →"** (hairline-stroke SVG arrow `.bf-submit__arrow`, not a glyph); **≤900px the label swaps to the short "Schedule →"** (`.bf-submit__label--short`, `aria-hidden` twin span — per Ben 2026-06-11, instead of collapsing the split) so it never wraps to 3 lines in the narrow panel; 900–~1180px the full label wraps to two lines (accepted, same tolerance as the old "Begin Practice" wrap). **Reassurance line** (`.bf-reassure`) under it, 12.5px Light ink-45 centered: booking "No obligation. Your details are never shared." / Self-led "We'll only email you about the self-led practice."
...
- **Nav/footer:** "Writing" link in the nav right cluster (before Client Portal) + first footer link. Active state on Writing pages only: 0.5px `--blue` underline (`.page-writing .nav__link--writing::after`). **⏸ Both links (nav + footer) + the mobile-menu link temporarily hidden 2026-06-22** (Astro `{/* */}` comments in `Nav.astro` ×2 + `Footer.astro`) until more essays publish (~2 weeks). The section, its routes, sitemap entries, RSS, and the active-underline CSS are all unchanged — only the on-site links are removed (kept-indexed decision). Reintroduce = un-comment the three links. See CONTEXT.md session 10.
- **Index (`wx__`):** eyebrow Mono 13 · h1 display `clamp(46,5.6vw,84)` "A practice of possibility" (per Ben 2026-06-11; was the comp's "Notes on practice") · lead "Essays on conscious change, leadership, and the practice of working with what comes." · lead sub-display `clamp(22,2.2vw,32)` max 760 · topic chips Mono 12, 4px radius, hover→blue, active ink-fill (client-side filter) · featured lead grid `1.15fr 1fr` gap 64 (4:3 image, blue Mono meta, title `clamp(32,3.4vw,46)`, dek 17/24, "Read the essay →" hairline-arrow link) · list rows grid `160px 1fr 200px` gap 48, 40px pad, title `clamp(24,2.4vw,32)` hover→blue, dek 15/22, row hover = 8px left-pad nudge. ≤900: single column.
...
- **Links** (Home `/` · Writing `/writing` · Contact `/contact` · Client Portal, external): vertically centered hairline list — HN Roman 400 36px / -0.025em white, 24px row padding, right-aligned DM Mono **11px** numerals `01–04` (white 0.45). **Current page's numeral = `--blue`** (set by JS from `location.pathname`). `:active` flashes the whole row blue (0.15s). **⏸ The "Writing" row is temporarily hidden 2026-06-22** (Astro `{/* */}` comment) until more essays publish (~2 weeks); the remaining rows are re-sequenced **Home 01 / Contact 02 / Client Portal 03** so the numerals stay gapless. Reintroduce = un-comment Writing (numeral 02) and renumber Contact → 03, Client Portal → 04. See CONTEXT.md session 10.
