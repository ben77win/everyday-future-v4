...
- Mobile: **tiles sit ABOVE the option cards** (gutter-inset group, `flex: 1` tiles, no scroll — see the tier-tiles note above; was a 72vw scroll-snap carousel below the cards); option cards are 2-up at ≤900px, and at ≤640px stack 1-up full-width, **fixed `height: 182px`** (uniform across all four — Orgs & Teams' 3-line desc sets the height; dropping the desktop square) with `28px` padding
