...
- Step 2: the unified `.bf-nav` (← Back → Step 1) + heading + offering detail block + form
  - **Layout (`.bf-step2-cols`, `grid-template-areas`) — reworked 2026-06-11 per Ben:** on desktop the **heading + sub + offering detail stack in the LEFT column** (columns `3fr 2fr`, areas `heading ./ sub ./ card formcol`) and the **"About you" + form sit in a WHITE BOX on the RIGHT** (`.bf-step2-formcol` — `background: var(--paper)`, `padding: 32px`, `margin-top: 40px` so its top aligns exactly with the offering block; the offering `.bf-step2-right` has the same 40px). The **offering detail lost its white card** — `.bf-offering-card` is now `background: transparent; padding: 0` (eyebrow/title/hairline grid sit directly on the cream). **Self-led** (no offering) toggles `.bf-step2-cols--nobox` via JS → reverts to the original single left-column stack (`2fr 3fr`, formcol under sub at `margin-top: 32px`, `.bf-step2-right` hidden) — no empty box column. _Known: the Begin Practice label wraps to two lines at mid-size viewports (~641–1000px) because of the box's 32px side padding — accepted for now._
  - **Mobile (≤640px):** single column, order **heading → sub → offering (on cream) → white form box**; formcol `margin-top: 0; padding: 20px` (inherits the old card's mobile padding); the offering detail grid is **collapsed by default** behind a "What's included +/−" toggle (`.bf-offering__toggle`, mobile-only — desktop always shows the grid). Toggle expands `.bf-offering--expanded .bf-offering-card__grid`.
