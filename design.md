...
- **Headline sizing:** `.bf-headline` (Step 1 "Choose how you want to practice." + Step 3 "Book your call"/"You're on the list.") uses the Display tier on desktop but **drops to the Sub-display tier on mobile** (`clamp(22px, 2.2vw, 32px)` / -0.010em / 1.2 → ~22px) so the flow's headings read as modest in-flow labels, not full page headlines. (Step 2's service-name `.bf-step2-heading` is separate, stays 34px mobile.)
- **Unified nav row (`.bf-nav`) on every step** — `← Back` (left, `.bf-back`) + `×` (right, `.bf-close`), hover → `--blue`. This is the *single* navigation for the whole flow (the old fixed drawer bar in `index.astro` was removed):
